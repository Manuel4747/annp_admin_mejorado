...
@NamedQueries({
    ...
    , @NamedQuery(name = "TipoModalidad.findById", query = "SELECT t FROM TipoModalidad t WHERE t.id = :id")
    , @NamedQuery(name = "TipoModalidad.findByDescripcion", query = "SELECT t FROM TipoModalidad t WHERE t.descripcion = :descripcion")})
public class TipoModalidad implements Serializable {
    ...
    @OneToMany(cascade = CascadeType.ALL, mappedBy = "tipoModalidad")
    private Collection<Formulario> formularioCollection;

    public TipoModalidad() {
    }

    public TipoModalidad(Integer id) {
        this.id = id;
    }

    public TipoModalidad(Integer id, String descripcion) {
        this.id = id;
    ...
    public String toString() {
        return "py.com.startic.gestion.model.TipoModalidad[ id=" + id + " ]";
    }

...
@Named(value = "bienesValidatorController")
@ViewScoped
public class BienesValidatorController extends AbstractController<Bienes>  implements Validator{
    ...
    public BienesValidatorController() {
    ...
    public void validate(FacesContext context, UIComponent component, Object submittedAndConvertedValue) throws ValidatorException {
        String rotulado = (String) submittedAndConvertedValue;

        if (rotulado == null || rotulado.isEmpty()) {
            return; // Let required="true" or @NotNull handle it.
        }
        
        Collection<Bienes> bienes = ejbFacade.getEntityManager().createNamedQuery("Bienes.findByRotulado", Bienes.class).setParameter("rotulado",rotulado).getResultList();

        if (bienes != null) {
            if(bienes.size() > 0 ){
                JsfUtil.addErrorMessage("Nro de Rotulado ya existe");
                throw new ValidatorException(new FacesMessage("Nro de Rotulado ya existe"));
            }
        }
    }

...
import py.com.annp.persistencia.models.FnDistribucion;
import jakarta.inject.Named;
import jakarta.faces.view.ViewScoped;
import jakarta.faces.context.FacesContext;
import jakarta.inject.Inject;

@Named(value = "fnDistribucionController")
@ViewScoped
public class FnDistribucionController extends AbstractController<FnDistribucion> {

    public FnDistribucionController() {
        // Inform the Abstract parent controller of the concrete FnDistribucion Entity
        super(FnDistribucion.class);
    }

    /**
     * Sets the "items" attribute with a collection of FnDetallePlanFinanciero
     * entities that are retrieved from FnDistribucion?cap_first and returns the
     * navigation outcome.
     *
    ...
    /**
     * Sets the "items" attribute with a collection of FnSubDistribucion
     * entities that are retrieved from FnDistribucion?cap_first and returns the
     * navigation outcome.
     *
     * @return navigation outcome for FnSubDistribucion page
     */
    public String navigateFnSubDistribucionCollection() {
        if (this.getSelected() != null) {
            FacesContext.getCurrentInstance().getExternalContext().getRequestMap().put("FnSubDistribucion_items", this.getSelected().getFnSubDistribucionCollection());
        }
        return "/pages/fnSubDistribucion/index";
    }
